...
本篇笔记我们将从概念、应用和原理两个方面来理解 PDF（概率密度函数）。
...
1. 我们需要知道概率密度函数与空间之间的关系。比如说我们现在要求解一个光线打到表面上时，反射光线在半球表面的分布。根据 Lambert's Cosine Law，我们设 $PDF=C \cdot cos\theta$。为什么要假设呢？因为我们知道了它与 $cos\theta$ 是成正比的，但是我们不知道它的系数是多少。而这个系数是为了让 PDF 在这个空间的积分结果为 1。通过这一点我们就可以求出 PDF 的解析式了；

2. 我们已经知道了 PDF 的解析式，所以说我们可以根据 PDF 来求出 CDF。求出 CDF 就相当于我们得到了一个 PDF 分布到均匀分布的映射关系；

3. 通过已知的「PDF 到均匀分布的映射关系」，我们对它求逆，就可以得到均匀分布到 PDF 分布的映射关系，并且可以求出解析式。所以说最后我们就可以通过「计算机极易实现的伪随机数生成器」来生成一个符合 PDF 分布的随机样本。
