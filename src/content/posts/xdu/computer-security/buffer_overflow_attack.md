...
2. 恶意程序的地址
   ...
   而得益于我们的 nop slide 设计，恶意程序的地址可以选择函数返回地址与恶意程序之间的任意位置（也就是说这个 `320 - len(shellcode)` 可以选择 0 到 `320 - len(shellcode)` 的任意数字）

不过需要注意的是，以上的结论均基于 gdb 调试工具。实际运行时，gdb 会向程序注入一些额外的代码和数据，因此栈也会更深。所以我们需要根据实际运行时的栈布局，调整其中的 +160 即 gdb 内容偏移量（需要多次尝试）；另外 +256 是为了直接进入通往恶意代码的 nop 滑梯，这儿的偏移量选取范围 +0 到 +start 之间均可。
...
最后一步只需要运行 python 代码生成恶意文件 `badfile`，然后运行用户程序，就可以看到我们进入了 zsh 的 shell 环境了！键入 `whoami` 看到输出为 `root` 就说明实验成功。

详细代码请移步👉[github/buffer-overflow](https://github.com/Yang-ZhiHang/buffer-overflow)

如果你有任何疑问或者对文章的勘误，欢迎联系 zamyang@qq.com！
